...
const executeHandler = async (robot, handler, data) => {
    ...
    const pipelineConfig = await getPipelineConfigFromApi(pipeline.name, pipeline.counter);
    const fullPipelineInfo = buildFullPipelineInfo(pipeline, pipelineConfig);
